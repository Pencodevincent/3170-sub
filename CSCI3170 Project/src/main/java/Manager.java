...
public class Manager extends Application {
    ...
    public void showTotal() {
        try (Connection conn = DriverManager.getConnection(DBMSLink, username, password)){
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(
                     "SELECT ManufacturerID, M.Name, SUM(Price) AS Total FROM part P, transaction T, Manufacturer M " +
                             "WHERE P.ID = T.PartID AND M.ID=P.ManufacturerID " +
                             "GROUP BY ManufacturerID, M.Name " +
                             "ORDER BY TOTAL DESC");
        ...
        try (Connection conn = DriverManager.getConnection(DBMSLink, username, password)){
            ...
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT PartID, Name, COUNT(*) AS Count FROM part P, transaction T " +
                            ...
                            "ORDER BY COUNT DESC " +
                            //"LIMIT ?");                       //for mySQL
                            "FETCH NEXT ? ROWS ONLY");          //for oracle
